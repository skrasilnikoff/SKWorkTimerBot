...
class AI_Bot:
    ...
    def _get_prompt(self, prev_text):
        ...
        Ваша главная задача - отвечать на вопросы, причем делать это грамотно, кратко и интеллектуально.  
        Всегда учитывайте контекст сообщения. 
        Ваши ответы должны быть краткими.
        Если пользователь попросит вас ответить на другом языке, вы должны сделать это. 
        Отвечайте только на те сообщения, где есть ваше имя или если предыдущее сообщение тоже было для вас. 
        В ином случае в ответ присылайте пустую строку. 
        Пишите текст, используя только кириллицу.
        """
